...
function gameOver(){
    ...
    const gameOverMessage = `Game Over! Your Final Score: <span class = 'badge'>${score}</span> Click 'Play Game' button to play again`
...
function chooseCard(card)
{
    if (canChooseCard())
    {
        ...
        setTimeout(() => {
            flipCards(false)
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Cards Revealed")
...
{
    return gameInProggress == true && !shufflingInProgress && !cardsRevealed
}
...
function checkForIncompleteGame(){
    ...
    if(serializedGameObj){
        ...
        if(gameObj.round >= maxRounds){
            ...
            if(confirm("Would you like to continue with your previous game?")){
                score = gameObj.score
...
function shuffleCards() {
    ...
    function shuffle() {
        ...
        if(shuffleCount == 500) {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Select the card which you think is the Ace of Spades........")
        }
        else{
            shuffleCount++;
        }
